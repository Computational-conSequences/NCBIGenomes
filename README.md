# NCBIGenomes
These two Gabo-friendly scripts help retrieve genomes from NCBI. I wrote them for myself, but might improve them as time permits (improvements from you will be appreciated).
...
2. bringMetadata.zsh
- this zsh script will download genome info from NCBI, using NCBI's "datasets" command, and extract a sublist of refseq genomes. For example, zsh bringMetadata.zsh Klebsiella will bring info from most Klebsiella genomes at NCBI, and extract a list of unique assembly identifiers.

3. bringGenomes.zsh
- this program will bring the genomes from the list, but check it, it expects the list to be in a directory produced by bringMetadata.zsh. If you ran the example command above, this one, with the same argument, will use the resulting list and download the corresponding Klebsiella genomes.
